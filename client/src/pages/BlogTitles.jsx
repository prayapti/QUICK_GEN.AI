...
const BlogTitles = () => {
    ...
    const blogCatogeries = [' General' , ' Technology', 'Buisness' ,'Health','Lifestyle' , 'Education ' ,
      'Travel' , 'Food'
    ];
  ...
  const {getToken} = useAuth()
    ...
    const onSubmitHandler = async(e) =>{
      ...
      try{
        ...
        setloading(true)
        const prompt= `Generate a blog title for keyword ${input} in the catogery ${selectedCatogery}` 
  ...
  return (
     <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap  gap-4 text-slate-700">
      ...
      <form onSubmit={ onSubmitHandler } className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
        ...
        <p className="mt-4 text-sm font-medium"> Catogery </p>

        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          { blogCatogeries.map((item, index) => (
            <span
              onClick={() => setSelectedCatogery (item)}
              key={index}
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${
    selectedCatogery === item
